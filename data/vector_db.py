# ==============================================================================
# 文件：data/vector_db.py[cite: 29]
# 职责：高性能本地向量数据库驱动 (Local Vector Database Driver)[cite: 29]
# 
# 详细功能介绍：
# 1. C 级极速检索：深度集成 `sqlite-vec` 底层扩展，利用 C 语言级的虚拟表进行余弦相似度（Cosine Distance）计算，实现毫秒级的向量匹配[cite: 29]。
# 2. 纯血 Python 双轨防御：针对部分安卓环境严格封锁 C 扩展加载权限的极端情况，自带一套纯 Python 的内存级计算降级方案，死保核心 RAG 检索功能不崩溃[cite: 29]。
# 3. 二进制序列化：将浮点数向量数组使用 `struct.pack` 强行压缩为极度紧凑的二进制 BLOB 格式，大幅缩减数据库的硬盘占用开销[cite: 29]。
# 4. 元数据(Meta)管理：内置 vdb_meta 字典表，记录建库进度、中断状态和总块数，为 UI 层的进度条断点续传提供物理级状态记录[cite: 29]。
#
# 架构定位：RAG（检索增强生成）系统的记忆中枢。负责将小说文本块和 AI 向量坐标进行永久固化和高速比对[cite: 29]。
# ==============================================================================
import sqlite3
import os
import struct
