...
class DialogManager:
    ...
    def show_changelog_dialog(app, e):
        ...
        log_text = """【v0.5.0】性能极限突围与架构稳固版
- (新增) 智能断点续传系统：深度集成 SQLite 状态机。若建库过程中止（闪退/杀后台），重启后将精准定位断点，自动跳过已处理切块，实现无损续传。
- (新增) 防 LMK 风险提示：在建库确认弹窗中加入硬件调度预警，引导用户在前台完成高强度计算，大幅降低安卓低内存杀手（LMK）误杀率。
- (优化) 核心引擎安全加固：
    * 引入“环形日志缓冲区 (deque)”：设定 500 条上限，彻底杜绝长时间运行下的日志内存膨胀。
    * 强化 Windows 容错：增加 API 返回条数校验，防御本地推理服务偶发的数据丢失。
    * 优化 Android 同步时序：调整 Vulkan 异步流水线与 Python 线程的协调机制，消除高频推理下的显存描述符池耗尽风险。
- (重构) AI 助手交互底盘：
    * 适配 Flet 0.84.0 选项卡架构：修复“人物/主线”等频道切换时的指示器漂移 Bug。
    * 独立频道记忆：每个分析模式（主线、人物、伏笔）现在拥有独立的追问历史，互不干扰。
    * 底栏交互瘦身：移除冗余按钮，释放更多阅读空间，提升聊天纵深感。
- (CI/CD) 构建链进化：
    * 引入 LunarG 官方源：Android 构建任务现在使用最纯正的 Vulkan SDK 进行着色器编译。
    * 环境隔离优化：实现 Windows 端 CUDA 巨量依赖的“云端拉取-动态注入-过河拆桥”瘦身战术。
【v0.4.6】端云混合 RAG 架构启航
