# ==============================================================================
# 文件：core/local_inference.py
# 职责：智能双核本地推理引擎 (Dual-Core Local Inference Engine)
# 
# 详细功能介绍：
# 1. 桌面端并行核心 (Win32)：调用 `llama-server.exe` 建立物理内存隔离的本地 HTTP 微服务。
#    通过 `n_parallel` 参数解锁多线程并发计算能力，彻底榨干 PC 端（如 RTX 4070）的极限算力。
# 2. 移动端原生核心 (NDK/Vulkan)：利用 `ctypes` 深度加载 `libllama.so` 动态库。
#    首创性地实现了在 Android 手机上直接运行 Vulkan GPU 加速。
# 3. 极限防爆缸机制：在移动端核心中，引入了 `n_ubatch` (物理运算切片) 和 
#    定向内存清理 (`llama_memory_seq_rm`) 机制，完美解决了手机由于可用显存过小
#    导致的大批量文本处理闪退问题。
# 4. 容错探针与日志：内置 C++ 层的日志回调探针，自动侦测设备是否具备 GPU 加速能力，
#    并在崩溃前一秒强行将堆栈信息写入物理黑匣子文件，为排查闪退提供最后遗言。
#
# 架构定位：这是一个高度硬件相关的底层驱动模块。它将复杂的 C/C++ 内存指针操作
# 封装成了极其简洁的 Python 接口 `get_embedding(text)` 供上层业务直接调用。
# ==============================================================================
import os
import sys
import json
