# ==============================================================================
# 文件：core/config_state.py
# 职责：全局配置与状态大管家 (Configuration & State Manager)
# 
# 详细功能介绍：
# 1. UI 偏好记忆：管理和持久化阅读器的字号、行高、间距、背景色、深浅色主题等视觉设置。
# 2. AI 核心枢纽：集中存储所有大模型相关的配置，包括 API 密钥、接口地址、以及底层本地引擎的硬核参数（并发数、GPU层数、物理切片大小等）。
# 3. 提示词司令部：内置并管理所有核心业务场景（主线总结、人物提取、深度剖析、伏笔探测、自由追问）的 System Prompt。
# 4. 状态持久化：与底层的 StorageManager 配合，确保用户的所有个性化软硬件设置在下次打开软件时完美还原。
#
# 架构定位：属于 App 底座的混入类 (Mixin)，充当整个软件极其关键的“配置数据中枢”。
# ==============================================================================
import sys
import flet as ft
