# ==============================================================================
# 文件：core/ai_service.py[cite: 31]
# 职责：AI 调度枢纽与网络通信基座 (AI Dispatcher & Network Service)[cite: 31]
# 
# 详细功能介绍：
# 1. 智能流量路由：作为中枢大脑，根据用户的配置，将大模型请求精准分发给“云端 API”或底层的“LocalEmbeddingEngine (本地推理引擎)”[cite: 31]。
# 2. 长连接防阻断：使用单例模式维护 `requests.Session` 并搭载底层重试适配器 (HTTPAdapter)。通过复用 TCP/SSL 握手，极大降低批量建库时的 API 延迟，并免疫部分大厂对高频请求的强制阻断 (10054 错误)[cite: 31]。
# 3. 流式对话引擎：底层手搓原生的流式 HTTP 解析器 (`stream_chat`)，按块解析 `text/event-stream` 格式的数据，驱动前端 UI 实现“打字机”效果[cite: 31]。
# 4. 异常兜底拦截：捕获深层的 HTTP 错误码（如 429 频控、400 格式错），并解包服务端的真实 JSON 报错原因，转化为对用户友好的界面提示[cite: 31]。
#
# 架构定位：软件与 AI 沟通的咽喉要道。所有的文本总结、向量转化、检索追问，都必须经过此文件的调度[cite: 31]。
# ==============================================================================
    ...
    def get_embedding(cls, config: dict, text: str) -> list[float]:
        ...
        if config.get("embed_mode") == "本地模型":
            ...
            try:
                ...
                hardware_mode = config.get("hardware_mode", "强制GPU模式")
                ...
                if cls._local_engine is not None:
                    ...
                    old_hw = getattr(cls._local_engine, 'hardware_mode', "强制GPU模式")
